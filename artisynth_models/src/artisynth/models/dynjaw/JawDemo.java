...
public class JawDemo extends RootModel implements ScalableUnits {
   ...
   protected String probesFilename = "incisorProbes.art";
   
   private class MouthOpeningAngle implements DataFunction{
      /*
       * Mouth opening angle is calculated as the angle between the upper and lower incisors in the saggital plane, in respect to the condylar centre point. 
       */
      public void eval(VectorNd vec, double t, double trel) {
                  String[] components = {"ltmj", "rtmj", "lowerincisor", "upperincisor"};
                  Point3d[] pts = new Point3d[4];
                                    
                  for(int i=0; i<components.length; i++){
                     String c_name = components[i];
                     pts[i] = ((FrameMarker) myJawModel.findComponent("frameMarkers/"+c_name)).getPosition().clone ();
                  }
                  
                  Point3d condyleCentre = new Point3d();
                  condyleCentre.add(pts[0], pts[1]).scale (0.5); // condyle center = 0.5 * (ltmj + rtmj)
                  condyleCentre.set(0, 0); // project onto sagittal plane
                  pts[2].set (0, 0); // project lower and upper incisors onto sagittal plane
                  pts[3].set (0, 0);
                  Vector3d li = new Point3d().sub (pts[2], condyleCentre);
                  Vector3d ui = new Point3d().sub (pts[3], condyleCentre);
                  
                  double angle = li.angle (ui);
                                
                  vec.set (0, Math.toDegrees(angle));
                }
   }
   
   /*
    * Mouth opening distance is calculated as inter-incisal distance.
    */
   private class MouthOpeningDistance implements DataFunction{
      
      public void eval(VectorNd vec, double t, double trel) {
         
         // calculate inter-incisal distance
         Point3d lower_incisor = ((FrameMarker) myJawModel.findComponent("frameMarkers/lowerincisor")).getPosition();
         Point3d upper_incisor = ((FrameMarker) myJawModel.findComponent("frameMarkers/upperincisor")).getPosition();
         
         double distance = new Vector3d().sub (lower_incisor, upper_incisor).norm ();       
   ...
   public void build (String[] args) throws IOException {
      ...
      //addConForceProbe(10, 0.01);
      File f = new File("jawMesh.obj");
      System.out.println(f.getAbsolutePath ());
      myJawModel.rigidBodies ().get ("jaw").getSurfaceMesh ().write (f);
   ...
   public void addMouthOpeningAngleProbe() {
      NumericMonitorProbe p = new NumericMonitorProbe (/*vsize=*/1, "mouthOpeningAngle.txt", 0, 10, 0.01);
      p.setDataFunction (new MouthOpeningAngle());
      p.setDefaultDisplayRange(-15, 15);
      p.setName("MouthOpeningAngle");
      addOutputProbe(p); 
   }
   
   public void addMouthOpeningDistanceProbe() {
      NumericMonitorProbe p = new NumericMonitorProbe (/*vsize=*/1, "mouthOpeningDistance.txt", 0, 10, 0.01);
      p.setDataFunction (new MouthOpeningDistance());
      p.setDefaultDisplayRange(0, 50);
      p.setName("MouthOpeningDistance");
      addOutputProbe(p); 
   }
   ...
   public void addConForceProbe(double duration, double interval) {
      ...
      System.out.println(myJawModel.bodyConnectors().toString ());
   ...
   // this is basically a copy of createIncisorPointForce()
...
//   }
   ...
   protected double bolusMaxResistance = 0.0; // N
